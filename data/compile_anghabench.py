...
import re
import argparse
from utils import remove_c_comments
...
def compile_and_write(input_file, output_file):
    ...
    input_text = remove_c_comments(input_text)
    try:
    ...
    sample = {
        "name": input_file.split('/AnghaBench/')[-1],
...
def main():
    ...
    if count:
        files = sorted(glob.glob(f"{root}/**/*.c", recursive=True))[: count]
    else:
        files = sorted(glob.glob(f"{root}/**/*.c", recursive=True))
...
if __name__ == "__main__":
    # download AnghaBench (https://github.com/brenocfg/AnghaBench) to a local folder
