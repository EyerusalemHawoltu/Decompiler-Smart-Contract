...
# Load train data
train_data_dirs = [
    '../data-tokenized/nova-anghabench-contra/train', '../data-tokenized/nova-the-stack-contra/train'
]
...
train_data.set_format(type='torch', columns=[
    'src-input_ids', 'src-labels', 'src-nova_attention_mask', 'asm-O0-input_ids', 'asm-O0-labels', 'asm-O0-nova_attention_mask', 'asm-O1-input_ids', 'asm-O1-labels', 'asm-O1-nova_attention_mask', 
    'asm-O2-input_ids', 'asm-O2-labels', 'asm-O2-nova_attention_mask', 'asm-O3-input_ids', 'asm-O3-labels', 'asm-O3-nova_attention_mask'])
...
for idx, data in enumerate(train_dataloader):
    ...
    nova_attention_mask = data['nova_attention_mask'].to(engine.device)
    ...
    nova_attention_mask = nova_attention_mask.view(bsz * gsz, L, L)

    lm_loss = 0
    hidden_states = []
    for i in range(bsz):
        output = engine(
            input_ids=input_ids[i * gsz: (i + 1) * gsz, :], attention_mask=attention_mask[i * gsz: (i + 1) * gsz, :], nova_attention_mask=nova_attention_mask[i * gsz: (i + 1) * gsz, :, :],
            labels=labels[i * gsz: (i + 1) * gsz, :], return_dict=True, output_hidden_states=True
        )
