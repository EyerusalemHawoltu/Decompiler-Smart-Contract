...
from modeling_nova import NovaModel, NovaForCausalLM
...
model = NovaForCausalLM.from_pretrained(model_load_folder, torch_dtype=torch.bfloat16).model
...
# Load train data
train_data_dirs = [
    '../data-tokenized/nova-binarycorp/train',
]
train_data = concatenate_datasets([
    load_from_disk(data_dir, keep_in_memory=True) for data_dir in train_data_dirs
]).shuffle(seed=7, keep_in_memory=True)
train_data.set_format(type='torch', columns=['asm-O0-input_ids', 'asm-O3-input_ids', 'asm-O0-nova_attention_mask', 'asm-O3-nova_attention_mask'])
...
if not torch.distributed.is_initialized() or torch.distributed.get_rank() == 0:
    ...
    print('Start Training, Gradient Checkpoint:', engine.module.gradient_checkpointing)
...
for epoch in range(2):
    for i, data in enumerate(train_dataloader):
        ...
        O0_nova_attention_mask = data['O0_nova_attention_mask'].to(engine.device)
        O3_nova_attention_mask = data['O3_nova_attention_mask'].to(engine.device)
        
        O0_h = engine.module(input_ids=O0_input_ids, attention_mask=O0_attention_mask, nova_attention_mask=O0_nova_attention_mask, return_dict=True, output_hidden_states=True).hidden_states[-1]
        O3_h = engine.module(input_ids=O3_input_ids, attention_mask=O3_attention_mask, nova_attention_mask=O3_nova_attention_mask, return_dict=True, output_hidden_states=True).hidden_states[-1]   # [B, L, H]
