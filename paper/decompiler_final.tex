...
\newcommand{\capstoneyear}[1]{}
\newcommand{\capstonedocument}[1]{}
\newcommand{\capstonesemester}[1]{}
...
\texttt{karim.ali@nyu.edu}
\and
Eyerusalem Hawoltu Afework\\
Computer Science, NYU Abu Dhabi\\
\texttt{eh3115@nyu.edu}
\and
Tamer Abdelaziz\\
Computer Science, NYU Abu Dhabi\\
\texttt{tamer.abdelaziz@nyu.edu}
}
...
Gigahorse~\cite{grech2019gigahorse} is a more recent declarative decompilation
framework that lifts EVM bytecode into a typed three-address code with explicit
data flow; we use it in Section~\ref{sec:gigahorse} as an alternative front-end to
our own CFG builder.
...
% ---------------------------------------------------------------------------
% ADDED: alternative Gigahorse front-end
% ---------------------------------------------------------------------------
\subsection{An Alternative Front-End: Gigahorse Three-Address Code}
\label{sec:gigahorse}

The CFG front-end of Section~\ref{sec:cfg-extract} exposes control flow but leaves
data flow implicit in the opcode stream. A natural question is whether a richer
intermediate representation one that makes data flow explicit gives the learned
stage more to work with. To answer this we add a second, \emph{interchangeable}
front-end based on Gigahorse~\cite{grech2019gigahorse}, a state-of-the-art
declarative static-analysis framework that lifts EVM bytecode into a typed
three-address code (TAC) with SSA-like variables, resolved function boundaries, and
named high-level operations.

We run Gigahorse over the same corpus inside a Singularity sandbox and harvest its
TAC relations (\texttt{TAC\_Op}, \texttt{TAC\_Block}, \texttt{TAC\_Def},
\texttt{TAC\_Use}, \texttt{InFunction}, and \texttt{HighLevelFunctionName}). A
serializer (\texttt{gigahorse\_tac\_serialize.py}) joins these relations into a
per-function TAC listing with canonical variable names (\texttt{v1}, \texttt{v2},
\dots), which becomes the model input in place of the CFG. Everything downstream is
held fixed: the same function-level alignment (by name and arity), the same Nova
model and hyperparameters (Section~\ref{sec:nova}), and the same data-cleaning
procedure. This design isolates the \emph{input representation} as the single
independent variable, so any difference in reconstruction quality is attributable
to the front-end alone.

\paragraph{Leakage-controlled comparison protocol.}
For this controlled comparison we tighten the data-splitting protocol. Because the
same Solidity function is frequently deployed across many contracts and recompiled
under several patch versions, a naive split leaks near-duplicate functions across
train and test. We therefore deduplicate by a \emph{pragma-invariant} key the
\texttt{md5} of the whitespace-normalized function body keeping a single
representative per unique body, and then split the unique bodies $60/20/20$ into
train/validation/test so that $\text{train}\cap\text{test}=\varnothing$ by
construction. The same procedure is applied identically to the CFG and Gigahorse
datasets (\texttt{prepare\_cfg\_dataset.py} and
\texttt{prepare\_gigahorse\_dataset.py}), yielding $22{,}735/7{,}578/7{,}578$ CFG
functions and $27{,}882/9{,}293/9{,}293$ Gigahorse functions. Absolute scores under
this stricter protocol are lower than the preliminary numbers of
Section~\ref{sec:nova-eval}, which did not deduplicate across splits; the protocol
here is designed for a fair \emph{relative} comparison of the two front-ends.
...
% ---------------------------------------------------------------------------
% ADDED: CFG vs Gigahorse front-end comparison
% ---------------------------------------------------------------------------
\section{Comparing the CFG and Gigahorse Front-Ends}
\label{sec:frontend-compare}

Using the leakage-controlled protocol of Section~\ref{sec:gigahorse}, we fine-tune
two Nova models that are identical in every respect except the input representation
one consuming the improved CFG, the other Gigahorse TAC and evaluate each on its
own held-out test split with greedy decoding.

\subsection{Metrics}
We report BLEU-4 and exact-match as before, and add a \emph{normalized-match}
metric that $\alpha$-renames identifiers (local variables and parameters) to a
canonical form before comparison, crediting reconstructions that are correct up to
variable naming which is fundamentally unrecoverable from bytecode. We rely on
normalized-match rather than CodeBERT cosine similarity for this comparison
because, in our domain, CodeBERT saturates: it assigns cosine similarities around
$0.97$ even to reconstructions that are clearly incorrect, so it cannot separate
good completions from bad ones.

\begin{table}[t]
\centering
\caption{CFG vs.\ Gigahorse front-ends under the leakage-controlled protocol
(Section~\ref{sec:gigahorse}). Each model is fine-tuned and evaluated on its own
deduplicated, leakage-free split; normalized-match credits reconstructions that
are correct up to identifier renaming.}
\label{tab:frontend}
\small
\begin{tabular}{l c c c c}
\toprule
Front-end & Test funcs & BLEU-4 & Exact & Norm.-match \\
\midrule
Improved CFG   & $7{,}578$ & \textbf{0.171} & 0.45\% & 6.55\% \\
Gigahorse TAC  & $9{,}293$ & 0.154 & \textbf{0.55\%} & \textbf{6.80\%} \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Result}
Table~\ref{tab:frontend} shows the two front-ends are statistically tied: the CFG
model attains BLEU-4 $0.171$ and the Gigahorse model $0.154$, with exact-match and
normalized-match differing by well under a percentage point in opposite directions.
Making data flow explicit via Gigahorse TAC does \emph{not} yield the improvement
one might expect. We read this as evidence that, at the $1.3$B-parameter scale, the
\emph{model capacity}, rather than the richness of the input representation, is the
binding constraint on reconstruction quality. The comparison also underscores the
importance of leakage-free splitting: the high absolute scores obtained under a
naive split (Section~\ref{sec:nova-eval}) are substantially inflated by
near-duplicate functions shared between train and test, and the deduplicated
figures here are a more faithful estimate of generalization. Together these
findings point toward larger models and semantics-aware training objectives as a
more promising direction than front-end engineering alone.
...
Beyond the CFG front-end, we implemented a second, interchangeable Gigahorse
three-address-code front-end and compared the two under a stricter,
leakage-controlled protocol. The two representations perform on par, indicating
that at the $1.3$B-parameter scale model capacity rather than input representation
is the binding constraint, and highlighting the importance of deduplicated,
